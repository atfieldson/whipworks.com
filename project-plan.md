...
- [x] **Commit 4: 2x3 Specialty Whips grid** — Indy, Catwhip, Zwhip, Nightlord, Mando, Harlequin (hover image-swap)
...
- [x] **Phase 13.8: Polish + mobile + a11y** *(feature/gallery-page branch)*
  - [x] **Scroll-triggered fade-up animation on gallery cards** — `motion(Card)` wrapper with `whileInView`. Cards fade up (24px y-translate + opacity) as they enter the viewport at 15% threshold, fire once each via `viewport: { once: true }`. Filter changes don't re-animate already-played cards (subsequent re-renders preserve the once-played state). *Decision note:* went with a uniform fade-up rather than the alternating left/right pattern from FeaturedSpecialtyGrid — at 35+ cards in the grid, alternating reads as chaotic; a single subtle fade-up keeps the rhythm cinematic.
  - [x] **Reduced-motion gating throughout** — `useReducedMotion()` honored across both `gallery.tsx` (cards) and `GalleryLightbox.tsx` (modal enter/exit). When set: card animations drop the y-translate to opacity-only; lightbox enter/exit skips slide-up + scale to opacity-only; focus-shift delay shortens 350ms → 50ms. Opacity transitions retained because they don't trigger vestibular-disorder symptoms — transforms (the WCAG-flagged motion type) are what's stripped.
  - [x] **Lightbox focus management** — on open, captures `document.activeElement` (typically the clicked card) and shifts focus to the close button after the entrance animation settles. On close, restores focus to the captured element so the user lands back exactly where they were in the grid. Wrapped in try/catch so a now-unfocusable previous element (e.g. filter narrowed it away) no-ops gracefully. Full Tab focus-trap intentionally deferred — Esc + click-backdrop + × button cover dismissal, and a true focus trap (focus-trap library or custom Tab-cycling) isn't justified for the polish budget here.
  - [x] **ItemList JSON-LD structured data on `/gallery`** — exposes the full whip inventory as a `Product`-typed list with name, image, url, brand. Built from the unfiltered card set (canonical gallery content is everything Adam has photographed; filtered view is a UX affordance, not what search engines should index). Per-item URL uses `card.href` so specialty whips link to `/specialty/:slug` and custom whips link to their prefilled designer page.
  - [x] **Image lazy-loading audit** — confirmed: gallery card photos use `loading="lazy"`, lightbox thumbnails use `loading="lazy"`, mobile photo stack uses `loading="lazy"`. Hero photo in lightbox is eager-loaded (correct UX — first thing the user sees on open).
  - [x] **Mobile responsive verified** across 320 / 375 / 414 viewports. Filter dropdowns wrap on narrow widths; lightbox uses vertical photo stack at ≤560px; hero+thumb-strip pattern hidden there; info panel pads down sensibly; gallery section margins halved on mobile (homepage convention).
  - [x] **Keyboard nav verified end-to-end**: Tab through cards → Enter opens lightbox → focus on close button → Tab cycles through thumbs / CTA / close → Esc closes → focus restored to triggering card. Filter dropdowns: Tab into trigger → Enter opens panel → Tab to checkboxes → Esc closes panel.
  - [x] TypeScript clean across all changes.
