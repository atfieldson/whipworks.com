...
export const resolveColorUrl = (waxed, color, isPrimary) => {
  if (!color) {
    return null;
  }
...
export const drawBullwhipPreviews = pattern => {
  ...
  let color1 = c.createPattern(primaryImage, 'repeat');
  let color2 = c.createPattern(secondaryImage, 'repeat');
  const bw = 400;
  const b16 = bw / 16;
