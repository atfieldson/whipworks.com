...
import {
  stockwhipGalleryItems,
  StockwhipGalleryWhip,
  StockwhipGalleryBreakImage,
} from './constants/galleryStockwhips';
...
type StockwhipImageProps = {
  src: string;
  alt: string;
  specs: StockwhipGalleryWhip['specs'];
  whipId: string;
};

export const StockwhipImage = ({ src, alt, specs, whipId }: StockwhipImageProps) => (
...
const StockwhipGallery = () => {
  const whips = stockwhipGalleryItems.filter(
    (item): item is StockwhipGalleryWhip => item.type === 'stockwhip'
  );

  const imageEntries = useMemo(() => {
    const entries: { whip: StockwhipGalleryWhip; angle: string; src: string }[] = [];
          ...
          <SimpleGrid columns={{ base: 1, sm: Math.min(row.columns, 2), md: row.columns }} spacing="3">
            {row.items.map((entry) => (
              <Box
                ...
                sx={entry.angle === 'wide1x1' ? { aspectRatio: '1 / 1' } : undefined}
                overflow="hidden"
                borderRadius="8px"
              >
                <StockwhipImage
...
export default StockwhipGallery;
